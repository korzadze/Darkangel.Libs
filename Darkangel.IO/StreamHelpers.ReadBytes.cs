...
namespace Darkangel.IO
{
    public static partial class StreamHelpers
    {
        ...
        {
            var buf = new byte[length];
            var cbRead = await stream.ReadAsync(buf, 0, length);
            if (cbRead != length) throw new EndOfStreamException();
            return buf;
        }
        /// <summary>
        ...
        /// <param name="length">Размер считываемых данных</param>
        /// <returns>Размер считанных данных</returns>
        public static async Task<long> ReadAsync(this Stream stream, byte[] data, long start, long length)
        {
            #region Check arguments
#if CHECK_ARGS
            _ = stream ?? throw new ArgumentNullException(nameof(stream));
            ...
            while (off < length)
            {
                ...
                var cbRead = await stream.ReadAsync(chunkBuf, 0, chunkSize);
                Array.Copy(chunkBuf, 0, data, start + off, cbRead);
                off += cbRead;
